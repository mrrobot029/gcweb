        ...
        case 'POST':
            ...
            switch ($req[0]) {
                ...
                case 'getConfirmedCount':
                    echo json_encode($post->executeWithRes("SELECT COUNT(gc_idnumber) as confirmedCount from tbl_gcat WHERE gc_status = 1 and gc_subdate != 0"));
                break;
                ...
                case 'getUnscheduledSubApplicants':
                    echo json_encode($post->executeWithRes("SELECT gc.*, si.si_email, si.si_mobile, si.si_firstname, si.si_lastname, CONCAT(si.si_lastname,', ',si.si_firstname,', ',si.si_midname,' ',si.si_extname)  as si_fullname FROM tbl_gcat as gc INNER JOIN tbl_studentinfo as si on si.si_idnumber = gc.gc_idnumber WHERE  gc.gc_subdate = '0' AND gc.gc_status = '1' ORDER BY $d->sort"));
                break;

                case 'getUnscheduledApplicants':
                    echo json_encode($post->executeWithRes("SELECT gc.*, si.si_email, si.si_mobile, si.si_firstname, si.si_lastname, CONCAT(si.si_lastname,', ',si.si_firstname,', ',si.si_midname,' ',si.si_extname)  as si_fullname FROM tbl_gcat as gc INNER JOIN tbl_studentinfo as si on si.si_idnumber = gc.gc_idnumber WHERE  gc.gc_status = '1' and gc.gc_subdate !=0 ORDER BY $d->sort"));
                break;
                ...
                case 'getUncheduledSubCount':
                    echo json_encode($post->executeWithRes("SELECT COUNT(gc_idnumber) as unscheduledSubCount FROM tbl_gcat WHERE gc_status = 1 AND gc_subdate = 0"));
                break;
                ...
                case 'unscheduleSubApplicant':
                    if($post->executeWithoutRes("UPDATE tbl_gcat SET gc_subdate = 0 WHERE gc_idnumber = '$d->gc_idnumber'")){
                        echo json_encode($post->executeWithoutRes("UPDATE tbl_gcatsubmitsched SET sub_count = sub_count-1 WHERE sub_recno = '$d->gc_subdate'"));
                    }
                 break;
                ...
                case 'addSubScheduleForApplicant':
                    echo json_encode($post->addSubScheduleForApplicant($d));
