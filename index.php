        ...
        case 'POST':
            ...
            switch ($req[0]) {
                ...
                case 'getUnscheduledApplicants':
                    $d = json_decode( base64_decode( file_get_contents('php://input')));
                    echo json_encode($post->executeWithRes("SELECT gc.*, si.si_email, si.si_mobile, CONCAT(si.si_lastname,', ',si.si_firstname,', ',si.si_midname,' ',si.si_extname)  as si_fullname FROM tbl_gcat as gc INNER JOIN tbl_studentinfo as si on si.si_idnumber = gc.gc_idnumber WHERE  gc.gc_examdate ='0000-00-00' && gc.gc_examtime='00:00:00' ORDER BY gc.gc_regtime ASC"));
                break;
                ...
                case 'searchUnscheduledApplicants':
                    $d = json_decode( base64_decode( file_get_contents('php://input')));
                    echo json_encode($post->executeWithRes("SELECT gc.*, si.si_email, si.si_mobile, CONCAT(si.si_lastname,', ',si.si_firstname,', ',si.si_midname,' ',si.si_extname)  as si_fullname FROM tbl_gcat as gc INNER JOIN tbl_studentinfo as si on si.si_idnumber = gc.gc_idnumber WHERE si.si_lastname LIKE '%$d->value%' || si.si_midname LIKE '%$d->value%' || si.si_firstname LIKE '%$d->value%' || gc.gc_idnumber LIKE '%$d->value%' || gc.gc_regtime LIKE '%$d->value%' || gc.gc_course LIKE '%$d->value%' && gc.gc_examdate ='0000-00-00' && gc.gc_examtime='00:00:00' ORDER BY gc.gc_regtime ASC"));
                break;
