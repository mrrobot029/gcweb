    ...
    class Post{
      ...
      GROUP BY es.es_sucode  ORDER BY cl.cl_block,su.su_code,cl.cl_day,cl.cl_stime,cl.cl_etime ASC");
                    ...
                    if($this->conn->query($updateNewStudent)){
                            $sqlgcat = "UPDATE tbl_gcat SET gc_course = '$course', gc_gpa = '$highschoolgpa', gc_english = '$english', gc_math = '$math', gc_science = '$science', gc_key = '$key' WHERE gc_idnumber = '$id'";
                                        if($this->conn->query($sqlgcat)){
                                            ...
                                            $mail->Subject = "Gordon College Admission Test(GCAT) Edit Information";
                                            $mail->Body = "Hello <b>{$fname} {$lname}</b>!<br>";
                                            $mail->Body .="Your information was updated successfully.<br>";
                                            $mail->Body .="This is your temporary id number: <b>{$id}</b>.<br><br>";
                                            $mail->Body .="<b>Confirm your registration and get your schedule for submission of requirements by clicking on this link:</b><br>";
                                            ...
                                            $mail->Body .="If you still need to edit your data, you can visit the link below:<br>";
        ...
        function insertNewStudent($d){
            ...
            function getApplicants($d) {
                return $this->executeWithRes("SELECT gc.*, si.si_email as si_email, si.si_mobile as si_mobile, si.si_firstname, si.si_lastname, CONCAT(si.si_lastname,', ',si.si_firstname,', ',si.si_midname,' ',si.si_extname) as si_fullname FROM tbl_gcat as gc INNER JOIN tbl_studentinfo as si on si.si_idnumber = gc.gc_idnumber ORDER BY $d->sort");
            }
            ...
            function validateEmail($d){
                return $this->executeWithRes("SELECT si_email from tbl_studentinfo WHERE si_email = '$d->si_email'");
            }
